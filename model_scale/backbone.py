...
class Feature_Extraction_2(torch.nn.Module):
    ...
    def __init__(self, in_channel, hidden_channel=64, n_rotation = 4, bone_kernel = [False, False], num_layers = 3, up_progress = True):
        ...
        if self.up_progress:
            ...
            # for i in range(2): 
            in_type = out_type
            out_type = nn.FieldType(self.r2_act, hidden_channel * 2 * [self.r2_act.regular_repr])
            self.up_layers_0 = nn.SequentialModule(
                    nn.R2Upsampling(in_type, 2),
                    nn.R2Conv(in_type, out_type, 1)
                )          
            self.up_layers_1 = Block(out_type, out_type)
            ...
            d1_regular_out_type = nn.FieldType(self.r2_act, hidden_channel * 2 * [self.r2_act.irrep(1)])
            self.layer1_out = nn.R2Conv(out_type, d1_regular_out_type, kernel_size= 3, stride = 1, padding = 1)
            # self.gpool = nn.GroupPooling(d1_regular_out_type) 
    ...
    def forward(self, input):
        ...
        if self.up_progress:
            ...
            #     x = _layer(x)
            #     # print(x.shape)
            x = self.up_layers_0(x)
            x = x + down_list[-2]
            x = self.up_layers_1(x)
            
            d1_out = self.layer1_out(x)#self.gpool(self.layer1_out(x))
        
            # feature_out1 = rearrange(d1_out.tensor, 'b c (d1 h) (d2 w) -> b (d1 d2) c h w', d1 = 4, d2 = 4) # 2 * (nume_layers -1) or 4
            feature_out1 = rearrange(d1_out.tensor, 'b (c d) h w -> b c d h w', d = self.hd)
            
            return feature_out3, feature_out1

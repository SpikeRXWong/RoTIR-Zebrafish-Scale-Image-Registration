...
def main(args):
    ...
    for file_name, file_path in zip(args.list, file_path_list):
        ...
        test_dict_whole = torch.load(os.path.join(args.folder,"final_test_datadict.pth"), map_location="cpu")
        ...
        for rotation_index in range(4):
            ...
            key = file_name + "_{:0>3d}".format((rotation_index + 1) * 90) 
            ...
            result_dict[key] = {
                ...
                "config" : "{}{}{}".format(
                    ...
                    "N" if (args.obligatory_mask == "none" and model_file['Apply_mask'] == 'none') or args.obligatory_mask == "false" else "T"# model_file['Apply_mask'][0].capitalize() 
                )
            }
    
            affine_matrix = matrix_calculation_function(output, score_thr, not use_scale, use_trans).to("cpu")
    ...
    torch.save(result_dict, os.path.join(args.folder, "direction_registraion_evaluation_{}.pth".format(os.environ['SLURM_JOBID'])))
